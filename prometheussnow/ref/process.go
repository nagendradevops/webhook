...
func processAlerts(data template.Alerts, partition string) error {
	...
	for _, alert := range alerts {
		...
		data := map[string]interface{}{
			"Urgency":           "3-Low",
			"Caller":            "Creator User",
			"short_description": fmt.Sprintf("recording event for => %s:%s:%s:%s", serviceKey, v["service"], v["facet"], location),
		}
